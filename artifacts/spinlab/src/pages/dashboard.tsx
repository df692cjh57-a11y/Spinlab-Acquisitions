...
import { ArrowRight, AlertCircle, TrendingUp, Ban, Activity, Users } from "lucide-react";
...
export default function Dashboard() {
  ...
  const stalledDeals = (allDeals as any[] ?? []).filter((d) => d.status === "Stalled");
        ...
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-border">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Overdue Items</span>
            </div>
        ...
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-border">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Recently Added Deals</span>
            </div>
            <Link href="/deals" className="text-xs text-primary hover:underline">View all</Link>
          </div>
        ...
        <div className="bg-card border border-border rounded-lg overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-border">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Stalled Deals</span>
            </div>
            <Link href="/deals" className="text-xs text-primary hover:underline">View all</Link>
          </div>
          {stalledDeals.length === 0 ? (
            <div className="px-4 py-6 text-sm text-muted-foreground text-center">No stalled deals.</div>
          ) : (
            <table className="w-full">
              <thead>
                <tr>
              ...
              <tbody>
                {stalledDeals.slice(0, 6).map((d: any) => (
                  <tr key={d.id} className="hover:bg-muted/40 transition-colors">
                    <td className="px-4">
                      <Link href={`/deals/${d.id}`}>
                        <div className="font-medium text-sm text-foreground hover:text-primary cursor-pointer">{d.dealName}</div>
